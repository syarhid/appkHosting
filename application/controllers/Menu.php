...
class Menu extends CI_Controller
{
	...
	function edit_menu()
	{
		...
		$this->form_validation->set_rules('menu1', 'Menu', 'required');
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->update('user_menu', ['menu' => $this->input->post('menu1')], ['menu' => $this->input->post('menu_sebelum')]);
	...
	function delete($menu)
	{
		...
		$this->db->delete('user_menu', ['menu' => $menu]);
	...
	function edit_submenu()
	{
		...
		$this->form_validation->set_rules('menu_id1', 'Menu', 'required');
		$this->form_validation->set_rules('url1', 'URL', 'required');
		$this->form_validation->set_rules('icon1', 'Icon', 'required');
		...
		if ($this->form_validation->run() == false) {
			...
			$data = [
				...
				'menu_id' => $this->input->post('menu_id1'),
				'url' => $this->input->post('url1'),
				'icon' => $this->input->post('icon1'),
				'is_active' => $this->input->post('is_active1')
			];

			$this->db->update('user_sub_menu', $data, ['title' => $this->input->post('title11')]);
